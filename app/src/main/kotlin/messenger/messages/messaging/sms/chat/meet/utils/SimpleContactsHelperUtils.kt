...
import messenger.messages.messaging.sms.chat.meet.model.Contact
import messenger.messages.messaging.sms.chat.meet.model.ContactsModel

class SimpleContactsHelperUtils(val context: Context) {
    fun getAvailableContacts(favoritesOnly: Boolean, getContacts: (ArrayList<ContactsModel>) -> Unit = {}): ArrayList<ContactsModel> {
...
//        callback(allContacts)
        getContacts.invoke(allContacts)
        return allContacts
    }
    ...
    fun getContacts(): List<Contact> {
        ...
        val PROJECTION = arrayOf(
            ...
            CommonDataKinds.Phone.ACCOUNT_TYPE_AND_DATA_SET,
            CommonDataKinds.Phone.NUMBER,
        ...
        val COLUMN_ID = 0
        val COLUMN_LOOKUP_KEY = 1
        val COLUMN_ACCOUNT_TYPE = 2
        val COLUMN_NUMBER = 3
        val COLUMN_TYPE = 4
        val COLUMN_LABEL = 5
        val COLUMN_DISPLAY_NAME = 6
        val COLUMN_PHOTO_URI = 7
        val COLUMN_STARRED = 8
        val CONTACT_LAST_UPDATED = 9

        context.queryCursor(URI, PROJECTION, null) { cursor ->
            contacts.add(Contact().apply {
                _id = cursor.getLong(COLUMN_ID)
                lookupKey = cursor.getString(COLUMN_LOOKUP_KEY)
                name = cursor.getString(COLUMN_DISPLAY_NAME) ?: ""
                photoUri = cursor.getString(COLUMN_PHOTO_URI)
                address = cursor.getString(COLUMN_NUMBER) ?: ""
            })
        }
        return contacts
    }
