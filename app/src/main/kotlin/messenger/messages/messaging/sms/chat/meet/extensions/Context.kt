...
import messenger.messages.messaging.sms.chat.meet.model.ThemeModel
import messenger.messages.messaging.sms.chat.meet.utils.*
import messenger.messages.messaging.sms.chat.meet.utils.MyContentProviderUtils.Companion.COL_ACCENT_COLOR
import messenger.messages.messaging.sms.chat.meet.utils.MyContentProviderUtils.Companion.COL_APP_ICON_COLOR
import messenger.messages.messaging.sms.chat.meet.utils.MyContentProviderUtils.Companion.COL_BACKGROUND_COLOR
import messenger.messages.messaging.sms.chat.meet.utils.MyContentProviderUtils.Companion.COL_LAST_UPDATED_TS
import messenger.messages.messaging.sms.chat.meet.utils.MyContentProviderUtils.Companion.COL_NAVIGATION_BAR_COLOR
import messenger.messages.messaging.sms.chat.meet.utils.MyContentProviderUtils.Companion.COL_PRIMARY_COLOR
import messenger.messages.messaging.sms.chat.meet.utils.MyContentProviderUtils.Companion.COL_TEXT_COLOR
...
fun Context.getPermissionString(id: Int) = when (id) {
    ...
    PERMISSION_READ_PHONE_NUMBER -> Manifest.permission.READ_PHONE_NUMBERS
    else -> ""
}
...
fun Context.getFontSizeText() = getString(
    when (mPref.fontSize) {
        FONT_SIZE_SMALL -> R.string.app_small
        FONT_SIZE_NORNAL -> R.string.app_normal
        FONT_SIZE_LARGE -> R.string.app_large
        else -> R.string.app_extra_large
    }
)

fun Context.getTextSize() =
    if (mPref.isUseSystemFont) {
        Log.d("TAG_FONT_SIZE", "getTextSize: ${FONT_SIZE_NORNAL}")
        resources.getDimension(R.dimen.textsize_16)
    } else {
        Log.d("TAG_FONT_SIZE", "getTextSize: ${mPref.fontSize}")
        when (mPref.fontSize) {
            FONT_SIZE_SMALL -> resources.getDimension(R.dimen.textsize_12)
            FONT_SIZE_NORNAL -> resources.getDimension(R.dimen.textsize_16)
            FONT_SIZE_LARGE -> resources.getDimension(R.dimen.textsize_18)
            else -> resources.getDimension(R.dimen.textsize_20)
        }
    }
...
@TargetApi(Build.VERSION_CODES.N)
fun Context.deleteBlockedNumber(number: Int) {
...
//    contentResolver.delete(BlockedNumbers.CONTENT_URI, selection, selectionArgs)
    CoroutineScope(Dispatchers.IO).launch {
        blockContactDao.deleteBlockedID(number)
    }
}
...
fun Context.isNumberBlockedNew(number: String): Boolean {
    val blockedNoList = blockContactDao.getAllBlockNo()
    blockedNoList.forEach {

        val blockNo = if (it.number.startsWith("91")) "+${it.number}" else it.number
        val nonBlockNo = if (number.startsWith("91")) "+${number}" else number
        Log.d("TAG_CHECK_BLOCK", "isNumberBlockedNew: $blockNo $nonBlockNo")
        if (nonBlockNo == blockNo) return true
    }
    return false
}
...
fun Context.isNumberBlocked2(threadID: Long, blockedNumbers: ArrayList<BlockContactModel> = getBlockedNumbers1()): Boolean {
    if (!isNougatPlus()) {
        return false
    }
    return blockedNumbers.map { it.threadID }.contains(threadID)
}
