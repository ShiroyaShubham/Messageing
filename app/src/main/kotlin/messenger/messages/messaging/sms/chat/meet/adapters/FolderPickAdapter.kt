...
import messenger.messages.messaging.sms.chat.meet.utils.getFilePlaceholderDrawables
import com.qtalk.recyclerviewfastscroller.RecyclerViewFastScroller
import messenger.messages.messaging.sms.chat.meet.views.CustomRecyclerView
import java.util.*

class FolderPickAdapter(
    mActivity: BaseActivity, val mFileDirItems: List<FileDIRModel>, recyclerView: CustomRecyclerView,
    itemClick: (Any) -> Unit,
            ...
            if (fileDirItem.isDirectory) {
                ...
                if (!mActivity.isDestroyed && !mActivity.isFinishing) {
                    ...
                    if (itemToLoad.toString().isGif()) {
                        Glide.with(mActivity).asBitmap().load(itemToLoad).apply(options).into(findViewById(R.id.ivThumb))
                    } else {
                        Glide.with(mActivity)
                            ...
                            .into(findViewById(R.id.ivThumb))
                    }
                }
            }
        }
